    ...
    const fetchLeave = async () => {
      try {
        const responnse = await axios.get(
          `https://employee-api-flax.vercel.app/api/leave/detail/${id}`,
          {
            headers: {
  ...
  const changeStatus = async (id, status) => {
    try {
        const responnse = await axios.put(
          `https://employee-api-flax.vercel.app/api/leave/${id}`, {status},
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
      ...
      {leave ? (
        <div className="max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <img
                src={`https://employee-api-flax.vercel.app/${leave.employeeId.userId.profileImage}`}
